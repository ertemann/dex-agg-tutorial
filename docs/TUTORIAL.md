...
> **💡 DEX/AMM Primer** (Skip if you're already familiar)
> 
> A decentralized exchange (DEX/AMM) has two core components: **Pools** and a **Router**. Pools are vaults holding 2+ tokens where the distribution determines trading prices - either by quantity ([XYK AMMs](https://medium.com/codex/an-introduction-to-automated-market-making-994bc76c61f4)) or by order placement (V3/Concentrated Liquidity). Pools are created permissionlessly by factory contracts for any token pair.
>
> The router finds the best price across all pools in a unified interface. All components are smart contracts with unique addresses (often deterministic for pools). Due to blockchain's integer-only math, tokens use **decimals** for float→int conversion (varies per token). Most DEX concepts originate from [Uniswap](https://docs.uniswap.org/sdk/v3/overview) - check their [contracts on Etherscan](https://etherscan.io/address/0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640#readContract) to learn more.
