...
class PairAPITest(APITestCase):
    ...
    def test_create_pair_via_api(self):
        ...
        created_pair = next(
            (p for p in get_response.data if p["pair_id"] == "LINKUSDC"), None
        )
        ...
        for sample_pair in self.sample_pairs:
            ...
            if "prices" in response.data and "best_price" in response.data:
                ...
                if prices:  # Only check if we have prices
                    ...
                    print(
                        f"{pair_id}: best_price ({best_price}) = min of {list(prices.values())}"
                    )
...
class PricingIntegrationTest(TestCase):
    ...
    def test_pairs_loaded_correctly(self):
        ...
        for pair in pairs:
            ...
            print(
                f"✓ {pair.pair_id}: {pair.base_token}({pair.base_token_decimals}) / {pair.quote_token}({pair.quote_token_decimals}) on {pair.active_exchanges}"
            )
